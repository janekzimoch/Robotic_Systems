...
volatile unsigned long R_ISR_current_time = 0;
volatile unsigned long R_ISR_previous_time = 0;
volatile float R_ISR_velocity = 0;
volatile unsigned long time_difference = 0;

volatile int extra_count = 0;


// ISR for Encoder_1 - Right Wheel
ISR( INT6_vect ) {

  // read new state of encoder pins
  boolean newE1_B = digitalRead( E1_B_PIN );
  boolean newE1_A = digitalRead( E1_A_PIN );

//  extra_count++;
//  if (extra_count >= 2){
//    R_ISR_previous_time = R_ISR_current_time;
//    R_ISR_current_time = micros();
//    time_difference = (R_ISR_current_time - R_ISR_previous_time);
//    R_ISR_velocity = extra_count*0.15 / (((float)time_difference)/1000000);  // units: [mm/s]
//    extra_count = 0;
//  }


  // restore true value of encoder
  newE1_A ^= newE1_B;
  ...
  // Determine direction of encoder - INEFFICIENT - may need changine later!
  switch ( state ) {
